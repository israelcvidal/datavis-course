...
function Network() {
  ...
  function setupData(data) {
    ...
    data.nodes.forEach(function(n){
      var randomnumber;
       // set initil xy to values vithin the width/height of the visualization
...
//    circleRadius = d3.scale.linear(countExtent);
  ...
  function updateNodes() {
    ...
    .attr("fill", function (d) { return '#f03b20'; })
  ...
  network = function(selection, data) {
    ...
    force.on("tick", forceTick).charge(-400).linkDistance(50);
    // perform rendering and start force layout 
    return force.start();

  };
    
  d3.select("#" + node).select("circle").style("fill", "#feb24c");
